...
func show_title() -> void:
	...
	v.add_child(_label("첫 마을에서 시작하는 반격. 챕터 1 '꺼진 봉화' — 농촌 → 창고 마을 → 고개 초소. 거점마다 방 던전(입구 → 전투 3 → 보스, 선택 보물방). 해방한 마을은 직접 걸어 다니며 개간·건설·관개·주민 배정으로 가꾼다.", 16))
	v.add_child(_label("HWR-005 · 기획 v0.6", 13, Color(0.7, 0.7, 0.7)))
	...
	v.add_child(HSeparator.new())
	v.add_child(_label("전투: 방향키 이동, X 평타, C 점프, Space 회피, A 돌진베기, S 올려베기, Enter 문 이동/상자, Esc 일시정지. 마을: 방향키/WASD 이동, E 작업, B 건설, 마우스 배치, R 회전, 우클릭/Esc 취소.", 13, Color(0.75, 0.75, 0.75)))
